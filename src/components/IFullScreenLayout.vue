    ...
    >
      ...
      <div v-show="!propData.displayContainer" class="fsl-box-body">
        <div
          ...
          :key="index"
          @click="gridElementClickHandle($event, getCurrentItem(item))"
          :style="{
            left: getCurrentItem(item).xRatio + '%',
            top: getCurrentItem(item).yRatio + '%',
            width: getCurrentItem(item).width + '%',
            height: getCurrentItem(item).height + '%',
            'border-width': (existsChooseGridList(item) ? 1 : 0) + 'px',
          }"
        >
          <!--布局开发层面-->
          <div
      ...
      <!--框选层-->
      <div
        v-show="!propData.displayContainer"
      ...
      <div class="fsl-layout-footer-switch-tool" v-if="moduleObject.env != 'production'">
        <div
          :class="{ active: layoutFooterMode === -1 }"
          @click="footerLayoutSwitch(null, -1)"
        >
          完整布局&nbsp;
          <a-tooltip placement="top">
            <template slot="title">
              <span
                >完整布局是指在最大分辨率情况下显示的布局，右侧能新增的自定义响应式布局只能在当前完整布局下删减布局，无法新增布局。如果完整布局发生变化，右侧自定义响应式布局可点击名称左边的重置功能图标进行重新布局。如果配置了响应式布局而实际条件不满足任何一个响应式布局时则同样显示该完整布局</span
              >
            </template>
        ...
        <div
          v-for="(fitem, findex) in chooseGridMediaList"
          :key="findex"
          @click="footerLayoutSwitch(fitem, findex)"
          :class="{ active: layoutFooterMode == findex }"
        >
          ...
          <a-tooltip placement="top" title="双击可修改宽高">
            <span v-show="!fitem.isEdit" @dblclick="editMediaWH(fitem, findex)"
              >&nbsp;宽>={{ fitem.w }},高>={{ fitem.h }}&nbsp;</span
            >
          </a-tooltip>
          <input
            v-show="fitem.isEdit"
            title="失去焦点或回车即可完成修改"
            :ref="`mediawhinput_` + findex"
            :value="fitem.w + ',' + fitem.h"
            @keyup.enter="mediaInputBlurHandle($event, fitem, findex)"
            @blur="mediaInputBlurHandle($event, fitem, findex)"
          />
        ...
        <a-tooltip
          placement="top"
          title="温馨提示：建议将完整布局完成后再新增自定义响应式布局"
        >
  ...
  methods: {
    ...
    initHandle() {
      if (this.propData.fitMenu) {
        this.computedWidth = {
          width: `calc(${this.propData.width} - ${this.propData.menuWidthMax})`,
        };
      }
      if (this.moduleObject.env === "develop") {
        const developEditInfo = IDM.develop.getDragWorkspaceInfo() || {};
        this.propData.fitMenu
          ? (this.computedWidth = {
              width: `calc(${developEditInfo.width}px - ${this.propData.menuWidthMax})`,
            })
          : (this.computedWidth = {});
      }
      this.chooseGridMediaList =
        (this.propData.chooseGridMediaList || []).length > 0
          ? this.propData.chooseGridMediaList
          : this.chooseGridMediaList;
    },
    ...
    mediaInputBlurHandle(e, item, index) {
      ...
      this.setPropDataToDevelopAttrData({
        chooseGridMediaList: _.cloneDeep(this.chooseGridMediaList),
      });
    },
    /**
     * 编辑布局的宽高
    ...
    addMediaObjectToList() {
      ...
      this.footerLayoutSwitch(
        this.chooseGridMediaList[this.chooseGridMediaList.length - 1],
        this.chooseGridMediaList.length - 1
      );
      this.setPropDataToDevelopAttrData({
        chooseGridMediaList: _.cloneDeep(this.chooseGridMediaList),
      });
    ...
    getCurrentUserRoleIds() {
      if (this.propData.userRoleFunction && this.propData.userRoleFunction.length > 0) {
        try {
          return window[this.propData.userRoleFunction[0].name].call(this, {
            customParam: this.propData.userRoleFunction[0].param,
            _this: this,
          });
        } catch (error) {
          return "";
    ...
    autoLayout() {
      ...
      for (let i = 0; i < this.propData.chooseGridMediaList.length; i++) {
        ...
        if (
          (whObject.width >= item.w || !item.w) &&
          (whObject.height >= item.h || !item.h)
        ) {
          if (item.w >= maxMediaObject.w && item.h >= maxMediaObject.h) {
            //如果宽度大于还不够，必须要高度也是大于才能将其替换
            if (item.isShowFunction && item.isShowFunction.length > 0) {
              // 判断如果页签有自定义函数 返回true 则替换
              if (
                window[item.isShowFunction[0].name] &&
                window[item.isShowFunction[0].name].call(this, {
                  customParam: item.isShowFunction[0].param,
                  item: item,
                  _this: this,
                })
              ) {
      ...
      this.$nextTick(function () {
        ...
        const refName =
          this.moduleObject.env == "production"
            ? "env_production_grid_"
            : "env_develop_grid_";
        gridList &&
          gridList.forEach((item) => {
            IDM.broadcast.sendChildren({
              type: "regionResize",
              message: {
                regionObject: _.cloneDeep(item),
                gridEleTarget:
                  this.$refs[refName + item.itemNo] instanceof Array
                    ? this.$refs[refName + item.itemNo][0]
                    : this.$refs[refName + item.itemNo],
                eleTarget:
                  this.$refs["region_element_" + item.itemNo] instanceof Array
                    ? this.$refs["region_element_" + item.itemNo][0]
                    : this.$refs["region_element_" + item.itemNo],
                outBoundingClientRect: _.cloneDeep(outBoxPos),
              },
              module: {
                packageid: this.moduleObject.packageid,
                containerIndex: item.itemNo,
              },
            });
          });
      });
    ...
    gridAnchorMousedownHandle(eT, item, type) {
      ...
      //单元格的宽与高
      const grid_width = outBoxPos.width / this.gridNumber,
        grid_height = outBoxPos.height / this.gridNumber;
      //进行x、y数据转换,因为窗口会缩小放大
      this.chooseGridList &&
      ...
      var moveEvent = function (e) {
        ...
        if (
          movePosObject.startX < 0 ||
          movePosObject.startY < 0 ||
          movePosObject.startX > outBoxPos.width ||
          movePosObject.startY > outBoxPos.height
        ) {
          return;
        }
        //正确的起始坐标(吸边效果)
        const newStartPos = that.getNewStartPos(
          movePosObject.startX,
          movePosObject.startY
        );
        //左上角坐标
        movePosObject.x = newStartPos.x;
        movePosObject.y = newStartPos.y;
        let moveBeforeStartX = -1,
          moveBeforeStartY = -1,
          moveBeforeEndX = -1,
          moveBeforeEndY = -1;
        //拿移动的对比
        switch (type) {
          case "topleft":
            ...
            if (
              movePosObject.y < moveBeforeStartY ||
              (Math.abs(movePosObject.startY - moveBeforeStartY) >= grid_height &&
                Math.round(item.height.toFixed(1)) > 100 / that.gridNumber)
            ) {
              //向上移动
            ...
            if (
              movePosObject.x < moveBeforeStartX ||
              (Math.abs(movePosObject.startX - moveBeforeStartX) >= grid_width &&
                Math.round(item.width.toFixed(1)) > 100 / that.gridNumber)
            ) {
              //向上移动
          ...
          case "topright":
            ...
            if (
              (movePosObject.x < moveBeforeEndX &&
                Math.round(item.width.toFixed(1)) > 100 / that.gridNumber) ||
              movePosObject.startX - moveBeforeEndX >= 0
            ) {
              //向上移动
              // item.x = movePosObject.x||0;
          ...
          case "bottomleft":
            ...
            if (
              (movePosObject.y < moveBeforeEndY &&
                Math.round(item.height.toFixed(1)) > 100 / that.gridNumber) ||
              movePosObject.startY - moveBeforeEndY >= 0
            ) {
              //向上移动
          ...
          case "center":
            ...
            if (
              Math.abs(mouseStartPosObject.startX - movePosObject.startX) >= grid_width &&
              ((mouseStartPosObject.startX > movePosObject.startX &&
                item.x - grid_width >= -2) ||
                (mouseStartPosObject.startX < movePosObject.startX &&
                  Math.round(
                    item.x + (item.width / 100) * outBoxPos.width + grid_width
                  ) <= outBoxPos.width))
            ) {
            ...
            if (
              Math.abs(mouseStartPosObject.startY - movePosObject.startY) >=
                grid_height &&
              ((mouseStartPosObject.startY > movePosObject.startY &&
                item.y - grid_height >= -2) ||
                (mouseStartPosObject.startY < movePosObject.startY &&
                  Math.round(
                    item.y + (item.height / 100) * outBoxPos.height + grid_height
                  ) <= outBoxPos.height))
            ) {
      ...
      var moveUpEvent = function (e) {
        ...
        that.chooseGridListFull &&
          that.chooseGridListFull.forEach((item) => {
            item.moveActive = false;
          });
    ...
    switchItemObjectCross(itemObject) {
      ...
      this.chooseGridList &&
        this.chooseGridList.forEach((item) => {
          ...
          //左上角坐标：item.x,item.y
          if (
            itemObject.x < item.x + this.regionDiffNumPX &&
            item.x + this.regionDiffNumPX < itemObjectEndX &&
            itemObject.y < item.y + this.regionDiffNumPX &&
            item.y + this.regionDiffNumPX < itemObjectEndY
          ) {
            topLeft = true;
          }
          //右上角坐标：itemEndX,item.y
          if (
            itemObject.x < itemEndX - this.regionDiffNumPX &&
            itemEndX - this.regionDiffNumPX < itemObjectEndX &&
            itemObject.y < item.y + this.regionDiffNumPX &&
            item.y + this.regionDiffNumPX < itemObjectEndY
          ) {
            topRight = true;
          ...
          if (
            itemObject.x < item.x + this.regionDiffNumPX &&
            item.x + this.regionDiffNumPX < itemObjectEndX &&
            itemObject.y < itemEndY - this.regionDiffNumPX &&
            itemEndY - this.regionDiffNumPX < itemObjectEndY
          ) {
            bottomLeft = true;
          }
          ...
          if (
            itemObject.x > item.x - this.regionDiffNumPX &&
            itemObject.x < itemEndX - this.regionDiffNumPX &&
            itemObjectEndY > item.y + this.regionDiffNumPX &&
            itemObjectEndY < itemEndY + this.regionDiffNumPX
          ) {
            bottomLeft = true;
          }
          //右上角坐标：itemObjectEndX,itemObject.y
          if (
            itemObjectEndX > item.x + this.regionDiffNumPX &&
            itemObjectEndX < itemEndX + this.regionDiffNumPX &&
            itemObject.y > item.y - this.regionDiffNumPX &&
            itemObject.y < itemEndY - this.regionDiffNumPX
          ) {
            topRight = true;
        ...
        */
          if (
            itemObject.x < item.x &&
            itemObjectEndX > itemEndX &&
            item.y < itemObject.y &&
            itemEndY > itemObjectEndY
          ) {
            cross = true;
          }
        ...
        */
          if (
            item.x < itemObject.x &&
            itemEndX > itemObjectEndX &&
            itemObject.y < item.y &&
            itemObjectEndY > itemEndY
          ) {
            cross = true;
          }
    ...
    resetGridLayoutPos(
      ...
      if (moveBeforeStartX > -1) {
        ...
        if (item.x < moveBeforeStartX) {
          ...
          item.width =
            item.width +
            (Math.abs(moveBeforeStartX - item.x) / grid_width) * (100 / that.gridNumber);
        } else {
          //减少
          item.width =
            item.width -
      ...
      if (moveBeforeStartY > -1) {
        ...
        if (item.y < moveBeforeStartY) {
          ...
          item.height =
            item.height +
            (Math.abs(moveBeforeStartY - item.y) / grid_height) * (100 / that.gridNumber);
        } else {
          //减少
          item.height =
            item.height -
      ...
      if (moveBeforeEndX > -1) {
        ...
        if (itemEndX < moveBeforeEndX) {
          ...
          item.width =
            item.width +
            (Math.abs(moveBeforeEndX - itemEndX) / grid_width) * (100 / that.gridNumber);
        } else {
          //减少
          item.width =
            item.width -
      ...
      if (moveBeforeEndY > -1) {
        ...
        if (itemEndY < moveBeforeEndY) {
          ...
          item.height =
            item.height +
            (Math.abs(moveBeforeEndY - itemEndY) / grid_height) * (100 / that.gridNumber);
        } else {
          //减少
          item.height =
            item.height -
    ...
    gridMousedownHandle(eT) {
      ...
      //正确的起始坐标(吸边效果)
      const newStartPos = this.getNewStartPos(
        that.movePosObject.startX,
        that.movePosObject.startY
      );
      ...
      //未移动点击的时候也要计算
      var resChooseWH = that.getChooseWH(
        that.movePosObject.startX,
        that.movePosObject.startY,
        { x: that.movePosObject.startX, y: that.movePosObject.startY }
      );
    ...
    switchAddGridList(e, isAdd) {
      ...
      that.movePosObject.xRatio = _.cloneDeep(
        (resNewStartPos.x / grid_width) * (100 / that.gridNumber)
      );
      that.movePosObject.yRatio = _.cloneDeep(
        (resNewStartPos.y / grid_height) * (100 / that.gridNumber)
      );
    ...
    hasRegionExistsGridLayout(startPos, gridWH) {
      ...
      this.chooseGridList.forEach((item) => {
        ...
        //左上角坐标：item.x,item.y
        if (
          startPos.x < item.x + this.regionDiffNumPX &&
          item.x + this.regionDiffNumPX < regionEndX &&
          startPos.y < item.y + this.regionDiffNumPX &&
          item.y + this.regionDiffNumPX < regionEndY
        ) {
          topLeft = true;
        }
        //右上角坐标：itemEndX,item.y
        if (
          startPos.x < itemEndX - this.regionDiffNumPX &&
          itemEndX - this.regionDiffNumPX < regionEndX &&
          startPos.y < item.y + this.regionDiffNumPX &&
          item.y + this.regionDiffNumPX < regionEndY
        ) {
          topRight = true;
        ...
        if (
          startPos.x < item.x + this.regionDiffNumPX &&
          item.x + this.regionDiffNumPX < regionEndX &&
          startPos.y < itemEndY - this.regionDiffNumPX &&
          itemEndY - this.regionDiffNumPX < regionEndY
        ) {
          bottomLeft = true;
        }
        //右下角坐标：itemEndX,itemEndY
    ...
    hasExistsGridLayout(position) {
      ...
      this.chooseGridList.forEach((item) => {
        if (
          item.x < position.x &&
          position.x < item.x + (item.width / 100) * outBoxPos.width &&
          item.y < position.y &&
          position.y < item.y + (item.height / 100) * outBoxPos.height
        ) {
          result = true;
        }
      });
    ...
    getChooseWH(startX, startY, mouseCurrentPos) {
      //最后真实的起始的X轴坐标，Y轴坐标
      const x = this.movePosObject.x,
        y = this.movePosObject.y;
      //最外层盒子的坐标
      const outBoxPos = this.$refs.refFslBgGrid.getBoundingClientRect();
      //存储x，y的坐标数组，grid_width单元格的宽，grid_height单元格的高
      let x_coord_array = [],
        y_coord_array = [],
        grid_width = outBoxPos.width / this.gridNumber,
        grid_height = outBoxPos.height / this.gridNumber;
      ...
      for (let index = 0; index < x_coord_array.length; index++) {
        ...
        if (
          element >= x &&
          element <= (startX > mouseCurrentPos.x ? startX : mouseCurrentPos.x)
        ) {
          xGridCount++;
        }
      }
      ...
      for (let index = 0; index < y_coord_array.length; index++) {
        ...
        if (
          element >= y &&
          element <= (startY > mouseCurrentPos.y ? startY : mouseCurrentPos.y)
        ) {
          yGridCount++;
        }
      }
      let regionWidth = xGridCount * (100 / this.gridNumber),
        regionHeight = yGridCount * (100 / this.gridNumber);
      //这里要判断最大宽与高，否则会重叠
      //区域的结束坐标
      const regionStartX = x < startX ? x : startX,
        regionStartY = y < startY ? y : startY,
        regionEndX = regionStartX + regionWidth,
        regionEndY = regionStartY + regionHeight;
      this.chooseGridList.forEach((item) => {
        //item的结束X坐标
        const itemEndX = item.x + (item.width / 100) * outBoxPos.width,
          //item的结束Y坐标
        ...
        */
        if (
          regionEndX - this.regionDiffNumPX > item.x &&
          regionEndX - this.regionDiffNumPX < itemEndX &&
          regionEndY - this.regionDiffNumPX > item.y &&
          regionEndY - this.regionDiffNumPX < itemEndY &&
          regionStartY - this.regionDiffNumPX > item.y &&
          regionStartY - this.regionDiffNumPX < itemEndY
        ) {
        ...
        */
        if (
          ...
          regionEndY - this.regionDiffNumPX > item.y &&
          regionEndY - this.regionDiffNumPX < itemEndY &&
          regionStartX - this.regionDiffNumPX > item.x &&
          regionStartX - this.regionDiffNumPX < itemEndX
        ) {
    ...
    getNewStartPos(x, y) {
      ...
      let startX = x_coord_array[0],
        startY = y_coord_array[0];
      for (let index = 0; index < x_coord_array.length; index++) {
        const element = x_coord_array[index];
        if (x < element) {
          //找到了比这个更大的就取上一个节点
    ...
    receiveBroadcastMessage(object) {
      switch (object.type) {
        ...
        case "changeMenuCollapse":
          if (this.propData.fitMenu) {
            this.computedWidth = (object.message || {}).menuCollapse
              ? { width: `calc(${this.propData.width} - ${this.propData.menuWidthMin})` }
              : { width: `calc(${this.propData.width} - ${this.propData.menuWidthMax})` };
          }
          break;
